...
const UIComponent: FC = () => {
  ...
  const [clickedMeshName, setClickedMeshName] = useState<string | undefined>(
    undefined
  );
  const [clickedMeshColor, setClickedMeshColor] = useState<string | undefined>(
    undefined
  );
  const [boxes, setBoxes] = useState(DEFAULT_BOXES);
  const [sceneClearColor, setSceneClearColor] = useState(
  ...
  const hideModal = () => {
    ...
    const animationClose = new Animation(
      "tv-off-1984",
      "scaling.x",
      10,
    ...
    const easingFunction = new ExponentialEase(9.7); // BABYLON.QuarticEase()
